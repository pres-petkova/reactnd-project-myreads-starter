...
class Book extends Component {
  ...
  handleBookUpdate = (event, book) => {
    ...
    update(book, newShelf).then((res) => {
      this.setState({
        book: { ...book, shelf: newShelf },
      });

      if (onBookUpdate) onBookUpdate(res);
    });
  };
  ...
  renderOption = (shelf, currentShelf, shelfTitle) => {
    if (shelf === currentShelf) {
      return (
        <option className="current-option" value={shelf}>
          {shelfTitle}
        </option>
      );
    } else {
        ...
        <div className="book-top">
          ...
          <div className="book-shelf-changer">
            <select onChange={(event) => this.handleBookUpdate(event, book)}>
              ...
              {this.renderOption(
                BOOK_SHELVES.CURRENTLY_READING,
                book.shelf,
                "Currently Reading"
              )}
              {this.renderOption(
                BOOK_SHELVES.WANT_TO_READ,
                book.shelf,
                "Want to Read"
              )}
